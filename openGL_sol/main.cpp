...
void divide_at_once(vector<Vector3> v, std::vector<Vector3> skeleton);
...
int main(int argc, char** argv)		//code6-volumetric_project.cpp
{
	//불러오고
	...
	{
		std::string obj_path = "./input/" + std::to_string(data_idx +i) + "/textured_model.obj";								//경로1/4
		string tex_path1 = "./input/" + std::to_string(data_idx + i) + "/textured_model_material0000_map_Kd.png";				//경로2/4
		string tex_path2 = "./input/" + std::to_string(data_idx + i) + "/textured_model_material0001_map_Kd.png";				//경로3/4
		string tex_path3 = "./input/" + std::to_string(data_idx + i) + "/textured_model_material0002_map_Kd.png";				//경로4/4
	...
	_mkdir(mkpath);
	sprintf_s(mkpath, "C:/02_data/05_skeleton/%s/part", cha);
	_mkdir(mkpath);

	//capture & run Openpose
	if (1)
	{
	...
	//상체하체 분리
	divide_at_once(sequence[1].v, point_vec2);
	//원하는 부위만 렌더링
	OBJViewer_part(data_num, sequence, tex_list, texture_num, cha, 1, bb);
...
void divide_at_once(vector<Vector3> v, std::vector<Vector3> skeleton)
{
	...
	for (int i = 0; i < v.size(); i++)
	{
		if (((v[i].x - skeleton[8].x) * spine.x + (v[i].y - skeleton[8].y) * spine.y + (v[i].z - skeleton[8].z) * spine.z) > 0)
		{
			...
			//내적값 구하기
			if (((v[i].x - start_p.x) * neck_born.x + (v[i].y - start_p.y) * neck_born.y + (v[i].z - start_p.z) * neck_born.z) > 0)
			{
				//head(1)
				bb.push_back(1);
			}
			else
			{
				...
				//8과 5의 b/a
				start_p1l.x = 4 * skeleton[5].x /5 +  skeleton[8].x / 5;		//앞 <-- 뒤 향하는 벡터
				start_p1l.y = 4 * skeleton[5].y /5 +  skeleton[8].y / 5;
				start_p1l.z = 4 * skeleton[5].z /5 +  skeleton[8].z / 5;
				...
				//내적값 구하기
				if (((v[i].x - start_p1.x) * r_shoulder_born.x + (v[i].y - start_p1.y) * r_shoulder_born.y + (v[i].z - start_p1.z) * r_shoulder_born.z) > 0)
				{
					//right
					...
					//내적값 구하기
					if (((v[i].x - start_p4.x) * r_hand_born.x + (v[i].y - start_p4.y) * r_hand_born.y + (v[i].z - start_p4.z) * r_hand_born.z) > 0)
					{
						//right hand
						bb.push_back(14);

					}
					else if (((v[i].x - start_p3.x) * r_d_arm_born.x + (v[i].y - start_p3.y) * r_d_arm_born.y + (v[i].z - start_p3.z) * r_d_arm_born.z) > 0)
					{
						//right lower arm
						bb.push_back(4);

					}
					else if (((v[i].x - start_p2.x) * r_up_arm_born.x + (v[i].y - start_p2.y) * r_up_arm_born.y + (v[i].z - start_p2.z) * r_up_arm_born.z) > 0)
					{
						//right upper arm
						bb.push_back(3);
				...
				else if(((v[i].x - start_p1l.x) * l_shoulder_born.x + (v[i].y - start_p1l.y) * l_shoulder_born.y + (v[i].z - start_p1l.z) * l_shoulder_born.z) > 0)
				{
					//왼팔 분류
					...
					//내적값 구하기
					if (((v[i].x - start_p4l.x) * l_hand_born.x + (v[i].y - start_p4l.y) * l_hand_born.y + (v[i].z - start_p4l.z) * l_hand_born.z) > 0)
					{
						//left hand
						bb.push_back(15);

					}
					else if (((v[i].x - start_p3l.x) * l_d_arm_born.x + (v[i].y - start_p3l.y) * l_d_arm_born.y + (v[i].z - start_p3l.z) * l_d_arm_born.z) > 0)
					{
						//left lower arm
						bb.push_back(7);
					...
					else if (((v[i].x - start_p2l.x) * l_up_arm_born.x + (v[i].y - start_p2l.y) * l_up_arm_born.y + (v[i].z - start_p2l.z) * l_up_arm_born.z) > 0)
					{
						//left upper arm
						bb.push_back(6);
